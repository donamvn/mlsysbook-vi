...
| **Nút thắt cổ chai** | Tính dễ vỡ\index{Brittleness!definition} (Các quy tắc bị phá vỡ) | Nhập liệu kiến thức (Chuyên gia khan hiếm) | Kỹ thuật đặc trưng (Tiền xử lý thủ công) | Quy mô tính toán & dữ liệu (Chi phí hạ tầng) |
...
| **machine learning thống kê (những năm 1990–2000)** | Kỹ thuật đặc trưng + học | Nhận dạng chữ số viết tay | khoảng 98–99% trên các benchmark thời MNIST [@lecun1998] | các pipeline đặc trưng thời CPU; tài nguyên thay đổi tùy theo triển khai |
| **deep learning (2012)** | mạng nơ-ron đầu cuối | Độ chính xác top-5 của ImageNet | `{python} AlexNetEvolutionTable.alexnet_top5_accuracy_str` (AlexNet) | 6 ngày trên 2 GPUs |
| **deep learning hiện đại (2020+)** | Transformers quy mô lớn | Độ chính xác top-1 của ImageNet | 88.55% (ViT-H/14) [@dosovitskiy2021image] | Tiền huấn luyện Bộ xử lý Tensor (TPU) quy mô lớn |
| **deep learning hiện đại (2023)** | Các mô hình nền tảng | benchmark MMLU | 86.4% (GPT-4) [@openai2023gpt4] | Ước tính ~`{python} GPT4ScaleEstimate.a100_gpu_days_million_str` (khoảng `{python} GPT4ScaleEstimate.a100_class_gpu_count_str` GPUs tham chiếu chạy trong `{python} GPT4ScaleEstimate.training_days_str`) [@semianalysisGPT4] |
...
|:-------------|:---------------------------------|:----------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------|:-------------------------------|
| **Đám mây** | Nút bộ tăng tốc trung tâm dữ liệu | Bộ nhớ thiết bị lớn cộng với bộ lưu trữ (`{python} DeploymentSystems.cloud_mem_math`) | Cấp thông lượng cao nhất (`{python} DeploymentSystems.cloud_ops_math`) | Nguồn điện được quản lý bởi cơ sở |
| **edge** | Robot học hoặc cổng công nghiệp | Bộ nhớ cục bộ dưới giới hạn triển khai (`{python} DeploymentSystems.edge_mem_math`) | Ngân sách bộ tăng tốc cục bộ hoặc CPU (`{python} DeploymentSystems.edge_ops_math`) | Nguồn điện tường, xe cộ hoặc tại chỗ |
| **Di động** | SoC cấp điện thoại thông minh hoặc thiết bị đeo | Bộ nhớ ứng dụng dùng chung (`{python} DeploymentSystems.mobile_mem_math`) | Các engine nơ-ron, GPU và CPU cấp điện thoại (`{python} DeploymentSystems.mobile_ops_math`) | Giới hạn pin và nhiệt |
| **TinyML** | Nút vi điều khiển | Bộ nhớ quy mô kilobyte (`{python} DeploymentSystems.tiny_mem_math`) | Tính toán cảm biến luôn bật (`{python} DeploymentSystems.tiny_ops_math`) | Ngân sách pin cấp milliwatt |
...
| **MobileNetV2**\index{MobileNetV2!definition} | Độ trễ và công suất | Thiết kế toán tử hiệu quả | Hệ thống có thể đáp ứng các ràng buộc thời gian thực về pin không? |
| **Keyword spotting**\index{Keyword spotting!definition} | Giới hạn công suất | Ngân sách bộ nhớ và năng lượng nhỏ | Hệ thống có thể chạy suy luận luôn bật trên milliwatt không? |
...
|:------------------------------------------------------------------|:------------------------------------|:--------------------------------------------------|:-----------------------------------------------------------------------------------|
| Frontier training | Cụm đám mây | GPT-4 | Mục tiêu: `{python} ScenarioRegistry.frontier_limit_str` |
| Nhận thức tự hành | Robot học edge | YOLOv8-nano | SLA (Service-Level Agreement): `{python} ScenarioRegistry.drive_limit_str` độ trễ |
| Trợ lý di động | Smartphone | LLM (Mô hình ngôn ngữ lớn) nhỏ, tối ưu cho di động | RAM: $< 2\text{ GB}$ / Nhiệt: $< 3\text{ W}$ |
| **Chuông cửa thông minh** | TinyML (MCU [Microcontroller Unit]) | Wake Vision | Công suất: `{python} ScenarioRegistry.doorbell_power_str` |
...
|:-------------------|:-------------------------------|:---------------------------------------------------------------------------------------------|
| **I: Nền tảng** | Bối cảnh: bức tranh tổng thể về hệ thống ML | Chương này, @sec-ml-systems, @sec-ml-workflow, @sec-data-engineering |
| **II: Xây dựng** | Lý thuyết: Nguyên tắc cơ bản của mô hình | @sec-neural-computation, @sec-network-architectures, @sec-ml-frameworks, @sec-model-training |
| **III: Tối ưu hóa** | Hiệu quả: Điều chỉnh hiệu suất | @sec-data-selection, @sec-model-compression, @sec-hardware-acceleration, @sec-benchmarking |
| **IV: Triển khai** | Sản xuất: Hệ thống thực tế | @sec-model-serving, @sec-ml-operations, @sec-responsible-engineering, @sec-conclusion |
