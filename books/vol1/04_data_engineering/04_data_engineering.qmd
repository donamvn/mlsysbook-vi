...
|:--------------------------------|:-------------------------------------------------:|:-----------------------------------------------:|
| **Phép nhân FP 32-bit** | `{python} EnergyAccessCosts.energy_flop_fp32_str` |                    1$\times$                    |
| **Truy cập bộ nhớ DRAM (32-bit)** |    `{python} EnergyAccessCosts.energy_dram_str`   |    `{python} EnergyAccessCosts.dram_mult_str`   |
| **Truy cập SSD cục bộ (32-bit)** | `{python} EnergyAccessCosts.local_ssd_energy_str` | `{python} EnergyAccessCosts.local_ssd_mult_str` |
| **Truyền qua mạng (32-bit)** |  `{python} EnergyAccessCosts.network_energy_str`  |  `{python} EnergyAccessCosts.network_mult_str`  |
...
|:------------------------------------|:-----------------------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------:|:-------------------------------------------------------------------------------------------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| **Lấy mẫu 16 kHz so với 8 kHz** | +`{python} KWSDesignSpaceTradeoffs.sampling_accuracy_gain_pct_str` độ chính xác |              `{python} KWSDesignSpaceTradeoffs.sampling_processing_mult_str` khối lượng công việc xử lý đầu vào |                                      `{python} KWSDesignSpaceTradeoffs.sampling_storage_mult_str` lưu trữ âm thanh thô | `{python} KWSDesignSpaceTradeoffs.sampling_feature_size_mult_str` kích thước đặc trưng |
| **13 so với 40 hệ số MFCC** | +`{python} KWSDesignSpaceTradeoffs.mfcc_accuracy_gain_pct_str` độ chính xác |                   `{python} KWSDesignSpaceTradeoffs.mfcc_feature_compute_mult_str` tính toán đặc trưng |                                                                         Tối thiểu | `{python} KWSDesignSpaceTradeoffs.mfcc_feature_memory_mult_str` bộ nhớ đặc trưng |
| **1M so với 10M ví dụ huấn luyện** | +`{python} KWSDesignSpaceTradeoffs.data_volume_accuracy_gain_pct_str` độ chính xác |                 `{python} KWSDesignSpaceTradeoffs.data_volume_training_time_mult_str` thời gian huấn luyện |                                   `{python} KWSDesignSpaceTradeoffs.data_volume_labeling_cost_mult_str` chi phí gán nhãn |    `{python} KWSDesignSpaceTradeoffs.data_volume_storage_mult_str` lưu trữ |
| **Dữ liệu huấn luyện sạch so với nhiễu** | +`{python} KWSDesignSpaceTradeoffs.noisy_accuracy_gain_pct_str` thực tế |                                                       Tối thiểu |                                    `{python} KWSDesignSpaceTradeoffs.noisy_collection_cost_mult_str` chi phí thu thập |                                    Tối thiểu |
| **Suy luận cục bộ so với đám mây** | lên đến `{python} KWSDesignSpaceTradeoffs.local_accuracy_risk_pct_str` rủi ro về độ chính xác | `{python} KWSDesignSpaceTradeoffs.local_latency_ms_str` so với `{python} KWSDesignSpaceTradeoffs.cloud_latency_ms_str` | `{python} KWSDesignSpaceTradeoffs.local_inference_cost_usd_per_query_str` so với `{python} KWSDesignSpaceTradeoffs.cloud_inference_cost_usd_per_query_str` |  `{python} KWSDesignSpaceTradeoffs.local_model_memory_kb_str` so với quy mô đám mây |
| **Tăng cường tổng hợp so với thực tế** | +`{python} KWSDesignSpaceTradeoffs.synthetic_robustness_gain_pct_str` tính mạnh mẽ |                                                       Tối thiểu |                                        `{python} KWSDesignSpaceTradeoffs.synthetic_cost_savings_mult_str` rẻ hơn |                                    Tối thiểu |
...
| **Suy luận** | Cục bộ, độ chính xác giảm | Độ chính xác giảm có thể giảm dung lượng; sự phù hợp 64 KB cuối cùng yêu cầu phân tích nén trong @sec-model-compression |
...
|:-----------------------------|:-----------------------------------------------------------------------:|:-----------------------|
| **Nhãn hình ảnh thu thập từ cộng đồng** |  `{python} DataEngineeringCostConstants.labeling_cost_crowd_range_str`  | Phân loại đơn giản |
| **Chú thích hộp giới hạn** |   `{python} DataEngineeringCostConstants.labeling_cost_box_range_str`   | Mỗi hộp, cảnh đơn giản |
| **Nhãn y tế chuyên gia** | `{python} DataEngineeringCostConstants.labeling_cost_medical_range_str` | Mỗi nghiên cứu, bác sĩ X-quang |
| **Lưu trữ S3 (Tiêu chuẩn)** |       `{python} DataEngineeringCostConstants.storage_cost_s3_str`       | Lưu trữ nóng |
| **Truy xuất S3 (Glacier)** |    `{python} DataEngineeringCostConstants.retrieval_cost_glacier_str`   | Tiêu chuẩn: 3-5 giờ |
...
|:-----------------------------------------|:----------------------:|:---------------------------------------------------------:|:---------------------------:|:----------------------------------------------|
| **NVMe SSD (cục bộ)** |       \$100–300        | `{python} KWSStorageEconomics.nvme_bw_range_gb_per_s_str` |          10–100 μs          | Tải dữ liệu huấn luyện, phục vụ (serving) đặc trưng hoạt động |
...
| **Danh mục nợ** | **Chỉ số** | **Ngưỡng cảnh báo** |   **Ngưỡng nghiêm trọng** |
|:------------------|:---------------------------------|:---------------------:|:--------------------------:|
| **Tài liệu** | % tập dữ liệu có thẻ dữ liệu |         < 80%         |           < 50%            |
| **Tài liệu** | % cột có mô tả |         < 90%         |           < 70%            |
| **Lược đồ** | Các nhánh phiên bản lược đồ |          > 3          |            > 10            |
| **Lược đồ** | % biến đổi có try/catch |         > 20%         |           > 50%            |
| **Chất lượng** | Tỷ lệ lỗi nhãn đã biết |          > 1%         |            > 5%            |
| **Chất lượng** | Các chỉ số độ chệch (bias) được ghi nhận |      Chưa đo lường | Đã đo lường nhưng chưa giảm thiểu |
| **Độ tươi mới** | Số ngày kể từ lần huấn luyện lại cuối cùng |       > 90 ngày |         > 365 ngày |
| **Độ tươi mới** | PSI so với đường cơ sở huấn luyện |         > 0.1         |           > 0.25           |
