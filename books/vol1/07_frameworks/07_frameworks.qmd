...
| **Khía cạnh** | **Dò vết (Tracing)** | **Scripting** |
|:----------------------|:-----------------------------|:------------------------------|
...
| **Khía cạnh** | **Eager + Autograd Tape** **(mặc định của PyTorch)** | **Đồ thị tĩnh**\index{Static graph!definition} **(TensorFlow&nbsp;1.x)** | **Biên dịch JIT** **(torch.compile)** |
|:-------------------------|:------------------------------------------------|:--------------------------------------------------------------------------|:----------------------------------------|
...
|:------------------------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------|
| **`retain_graph=True`**\index{Graph retention!multiple backward passes} | Giữ trạng thái đồ thị đã lưu có sẵn cho một lần duyệt ngược khác | Giữ lại bộ nhớ mà lẽ ra sẽ được giải phóng; lượng phụ thuộc vào đồ thị và các giá trị đã lưu |
| **Tích lũy gradient** | Các lượt lan truyền ngược liên tiếp cộng dồn vào `.grad` cho đến khi `zero_grad()` đặt lại chúng, cho phép các batch hiệu quả lớn | Quên đặt lại âm thầm trộn lẫn các gradient qua các bước tối ưu hóa |
| **`autograd.Function` tùy chỉnh** | Các quy tắc truyền xuôi và truyền ngược do người dùng định nghĩa cho các phép toán nằm ngoài thư viện tích hợp sẵn | Chuyển giao hợp đồng vi phân (những gì cần lưu, cách vi phân) cho người triển khai |
| **Hook gradient**\index{Gradient hook!inspection and modification} | Kiểm tra hoặc sửa đổi gradient giữa quá trình duyệt (cắt bớt, ghi nhật ký, gỡ lỗi) | Chạy mã Python tùy ý cho mỗi tensor đã đăng ký trên mỗi lượt lan truyền ngược |
| **`.detach()`**\index{Gradient flow!tensor detachment} | Cắt luồng gradient tại một ranh giới được chọn (các lớp đóng băng, đầu ra suy luận) | Thuộc tính `.data` cũ bỏ qua autograd và âm thầm làm hỏng gradient; nhân bản trước khi thay đổi tại chỗ |
...
|:-----------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------:|:-----------------------------------------------------:|:---------------|
| **PCIe 3.0 x16** |                                                         `{python} DeviceBandwidthHierarchy.pcie3_gb_per_s_str`                                                        |  `{python} DeviceBandwidthHierarchy.pcie3_4mb_ms_str` | Từ máy chủ đến thiết bị |
| **PCIe 4.0 x16** |                                                         `{python} DeviceBandwidthHierarchy.pcie4_gb_per_s_str`                                                        |  `{python} DeviceBandwidthHierarchy.pcie4_4mb_ms_str` | Từ máy chủ đến thiết bị |
| **NVLink 3.0** | `{python} DeviceBandwidthHierarchy.nvlink_a100_per_direction_gb_per_s_str` mỗi hướng (`{python} DeviceBandwidthHierarchy.nvlink_a100_gb_per_s_str` hai chiều) | `{python} DeviceBandwidthHierarchy.nvlink_4mb_ms_str` | Từ GPU đến GPU |
| **Bộ nhớ GPU** |                                                        `{python} DeviceBandwidthHierarchy.a100_bw_gb_per_s_str`                                                       |   `{python} DeviceBandwidthHierarchy.hbm_4mb_ms_str`  | Trên thiết bị |
...
| **JAX/Flax**\index{Flax!JAX neural network library} | `variables["params"]` sau `variables = model.init(key, x)` | Các tập hợp biến riêng biệt (ví dụ, `batch_stats`) |
...
| **Entropy chéo (đơn giản hóa)** |                           ~`{python} TrainingStepCalc.train_ce_kflop_str`                           |  `{python} TrainingStepCalc.train_ce_mem_kb_str`  |  `{python} TrainingStepCalc.train_ce_ai_str`  |
| **Lan truyền ngược (giả định 2$\times$ lan truyền tiến)** |                           ~`{python} TrainingStepCalc.train_bwd_mflop_str`                          |  `{python} TrainingStepCalc.train_bwd_mem_mb_str` |  `{python} TrainingStepCalc.train_bwd_ai_str` |
...
|:----------------------------------------------------|:-------------------------------------------------------|:------------------------------------------------------------------|
| **CUDA GPU**\index{GPU backend!triển khai CUDA} | cuBLAS [@nvidia_cublas; @choquette2021] | Các kernel GEMM và các luồng CUDA để thực thi bất đồng bộ |
| **CPU**\index{CPU backend!các thư viện BLAS được tối ưu hóa} | Intel oneMKL hoặc OpenBLAS [@intelOneMKL; @openblasDocs] | Song song hóa cấp độ luồng xung quanh các kernel được tối ưu hóa |
| **TPU**\index{TPU!biên dịch XLA} | XLA [@GoogleXLA] | Biên dịch thành các phép toán của bộ tối ưu hóa cấp cao (HLO) dành riêng cho TPU |
