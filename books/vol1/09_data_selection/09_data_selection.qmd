...
| **Tài nguyên** |                                                                                                   **Tốc độ tăng trưởng** | **Hàm ý** |
|:------------------------|------------------------------------------------------------------------------------------------------------------:|:----------------------------------------------------------------------------------|
| **Tính toán GPU** |               ~`{python} ScalingAsymmetry.gpu_growth_mult_str` / `{python} ScalingAsymmetry.gpu_period_years_str` | Thông lượng phần cứng có thể tăng nhanh trong một kỷ nguyên nhất định |
| **Dữ liệu huấn luyện (Web)** |     ~`{python} ScalingAsymmetry.web_data_growth_mult_str` / `{python} ScalingAsymmetry.web_data_period_years_str` | Văn bản web chất lượng cao là hữu hạn; phần lớn đã được thu thập |
| **Dữ liệu được gán nhãn** | ~`{python} ScalingAsymmetry.label_data_growth_mult_str` / `{python} ScalingAsymmetry.label_data_period_years_str` | Thông lượng chú thích của con người vốn dĩ bị giới hạn |
| **Dữ liệu tổng hợp** |                                                                                                 Có khả năng lớn | Bị giới hạn bởi chất lượng của bộ tạo (các mô hình được huấn luyện trên dữ liệu do mô hình tạo ra có thể suy giảm chất lượng) |
...
| **Khung nhìn machine learning** |                         **Khung nhìn hệ thống** |
|:--------------------------------------|--------------------------------------------:|
| **"Ít mẫu hơn để đạt cùng độ chính xác"** |             "Ít FLOPs hơn để đạt cùng độ chính xác" |
| **"Khả năng tổng quát hóa tốt hơn"** | "Chi phí huấn luyện thấp hơn (thời gian, tiền bạc, năng lượng)" |
| **"Giới hạn độ phức tạp của mẫu"** |            "Hiệu quả tài nguyên từ đầu đến cuối" |
| **"Lý thuyết học máy"** |                          "Kỹ thuật chi phí" |
...
| **Phương pháp** | **Chi phí tính toán** | **Yêu cầu huấn luyện** | **Tốt nhất cho** | **Hạn chế** |
|:---------------|:----------------------------------------|:----------------------|:----------------------|:--------------------------|
| **k-Center** | $\mathcal{O}(D^2)$ or $\mathcal{O}(DK)$ | Không | Độ bao phủ, khám phá | Bỏ qua thông tin nhãn |
| **Herding** | $\mathcal{O}(DK)$                       | Không | Khớp phân phối | Giả định giống Gaussian |
| **GraNd** | $\mathcal{O}(\text{epochs} \times D)$   | Có (vài epoch) | Đường biên quyết định | Yêu cầu huấn luyện proxy |
| **Quên** | $\mathcal{O}(\text{full training})$     | Có (đầy đủ) | Các ví dụ khó | Tốn kém để tính toán |
| **EL2N** | $\mathcal{O}(\text{epochs} \times D)$   | Có (vài epoch) | Lấy mẫu dựa trên sự không chắc chắn | Tốt nhất với mô hình proxy |
...
| **Chiến lược** | **Điểm độ khó** | **Tốt nhất cho** |
|:----------------------|:------------------------------------------|:--------------------------------------------|
| **Dựa trên hàm mất mát** | Hàm mất mát từ mô hình thăm dò (thấp = dễ) | Đa năng; yêu cầu huấn luyện mô hình thăm dò |
| **Dựa trên độ tin cậy** | Độ tin cậy của mô hình Teacher (cao = dễ) | Khi mô hình Teacher có sẵn; distillation setups |
| **Các phương pháp heuristic theo miền** | Độ dài câu, độ phức tạp của hình ảnh | Không cần tính toán bổ sung; yêu cầu kiến thức miền |
| **Tự điều chỉnh tốc độ** | Hàm mất mát của mô hình hiện tại (cập nhật mỗi epoch) | Thích ứng; không cần mô hình thăm dò |
...
| **Tập dữ liệu** | **Mô hình** | **Chiến lược điều chỉnh tốc độ** | **Số epoch để đạt độ chính xác mục tiêu** |                        **Giảm số epoch** |
|:--------------|:---------:|:-----------------------|:-------------------------------------------------------------------------------------------------------------------------------------------|:------------------------------------------------------------------:|
| **CIFAR-10** | ResNet-18 | Khởi động tuyến tính | `{python} CurriculumBenchmarks.cifar10_curriculum_epochs_str` so với `{python} CurriculumBenchmarks.cifar10_baseline_epochs_str` baseline |  `{python} CurriculumBenchmarks.cifar10_speedup_str` ít epoch hơn |
| **CIFAR-100** | ResNet-32 | Tự điều chỉnh tốc độ | `{python} CurriculumBenchmarks.cifar100_curriculum_epochs_str` so với `{python} CurriculumBenchmarks.cifar100_baseline_epochs_str` baseline | `{python} CurriculumBenchmarks.cifar100_speedup_str` ít epoch hơn |
| **ImageNet** | ResNet-50 | Dựa trên hàm mất mát | `{python} CurriculumBenchmarks.imagenet_curriculum_epochs_str` so với `{python} CurriculumBenchmarks.imagenet_baseline_epochs_str` baseline | `{python} CurriculumBenchmarks.imagenet_speedup_str` ít epoch hơn |
| **ImageNet** | ResNet-50 | Đã học (nhãn nhiễu) | `{python} CurriculumBenchmarks.mentornet_curriculum_epochs_str` so với `{python} CurriculumBenchmarks.mentornet_baseline_epochs_str` đường cơ sở | `{python} CurriculumBenchmarks.mentornet_speedup_str` ít epoch hơn |
...
| **Ngân sách nhãn** | **Phương pháp** |                           **Độ chính xác** |                                                                              **Hiệu quả nhãn** |
|:----------------------------------------------------------------------------------------------------------------------------------------------|:------------------|:---------------------------------------------------------------:|--------------------------------------------------------------------------------------------------:|
| **`{python} FixmatchLabelEfficiency.cifar10_full_labels_str` (`{python} FixmatchLabelEfficiency.cifar10_full_frac_pct_str`)**                 | Tập huấn luyện đầy đủ |                               ---                               |                                                                             Tham chiếu số lượng nhãn |
| **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_labels_str` (`{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_frac_pct_str`)**   | FixMatch |  `{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_acc_str` |  `{python} FixmatchLabelEfficiency.cifar10_fixmatch_4k_efficiency_factor_mult_str` hiệu quả hơn |
| **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_labels_str` (`{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_frac_pct_str`)** | FixMatch | `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_acc_str` | `{python} FixmatchLabelEfficiency.cifar10_fixmatch_250_efficiency_factor_mult_str` hiệu quả hơn |
| **`{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_labels_str` (`{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_frac_pct_str`)**   | FixMatch |  `{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_acc_str` |  `{python} FixmatchLabelEfficiency.cifar10_fixmatch_40_efficiency_factor_mult_str` hiệu quả hơn |
...
| **Phương thức** | **Tác vụ tự giám sát** |                      **Tín hiệu giám sát** |
|:----------------|:-------------------------|--------------------------------------------:|
| **Văn bản** | Mô hình hóa ngôn ngữ được che giấu |                 Dự đoán [MASK] từ ngữ cảnh |
| **Văn bản** | Dự đoán token tiếp theo |              Dự đoán token tiếp theo trong chuỗi |
| **Hình ảnh** | Học tương phản | Cùng một hình ảnh (được tăng cường) so với các hình ảnh khác nhau |
| **Hình ảnh** | Tự mã hóa được che giấu |                  Tái tạo các patch được che giấu |
| **Đa phương thức** | Căn chỉnh kiểu CLIP |                      Ghép nối các cặp hình ảnh-văn bản |
...
| **Phương pháp** | **Nhãn mỗi tác vụ** | **Tính toán mỗi tác vụ** |     **Thu thập dữ liệu** |
|:-------------------------------|:--------------------|:----------------------|-------------------------:|
| **Huấn luyện từ đầu** | 100K–1M được gán nhãn | 100% huấn luyện đầy đủ | Thu thập cụ thể cho tác vụ |
| **Tinh chỉnh mô hình nền tảng** | 100–1K được gán nhãn | 1–5% của huấn luyện đầy đủ | Tái sử dụng corpus tiền huấn luyện |
...
| **Tỷ lệ tổng hợp** | **Kết quả đại diện** |
|:-----------------------------|:----------------------------------------------------|
| **100% tổng hợp** | Không có điểm neo dữ liệu thực; xác thực khoảng cách miền |
| **80% tổng hợp + 20% thực** | Nhu cầu dữ liệu thực thấp hơn; xác thực trên dữ liệu triển khai |
| **50% tổng hợp + 50% thực** | Nhiều điểm neo dữ liệu thực hơn; chi phí thu thập cao hơn |
| **100% thực** | Không có khoảng cách từ tổng hợp đến thực; chi phí thu thập cao nhất |
...
| **Giai đoạn** | **Thời điểm áp dụng** | **Kỹ thuật** | **Kết quả minh họa** |
|:-------------------------|:-----------------|:------------------------------------------------------|:-----------------------------------------------------|
| **Tỉa (pruning) tĩnh** | Trước khi huấn luyện | Lựa chọn Coreset, Loại bỏ trùng lặp, Lọc chất lượng | Giảm tập dữ liệu 30–50% |
| **Lựa chọn động** | Trong quá trình huấn luyện | Curriculum Learning, Active Learning, Bán giám sát | Curricula nhanh hơn 10–30%; nhãn ít hơn 2--100$\times$ |
| **Tạo tổng hợp** | Theo yêu cầu | Tăng cường, Mô hình sinh tạo, Chưng cất | Mở rộng dữ liệu hiệu quả 2--10$\times$ |
...
| **Ràng buộc** | **Kỹ thuật tiềm năng** | **Lý do** |
|:-------------------------------|:--------------------------|:-----------------------------------------------------|
| **Ngân sách gán nhãn hạn chế** | Active Learning | Tối đa hóa ROI nhãn bằng cách chọn các mẫu thông tin |
| **Dư thừa cao trong dữ liệu** | Loại bỏ trùng lặp + Coreset | Loại bỏ lãng phí trước khi huấn luyện bắt đầu |
| **Các lớp hiếm hoặc các trường hợp biên** | Tạo tổng hợp | Tạo ra các mẫu không tồn tại trong dữ liệu thô |
| **Hội tụ chậm** | Curriculum Learning | Cải thiện chất lượng gradient trong giai đoạn huấn luyện ban đầu |
| **Yêu cầu về quyền riêng tư** | Tổng hợp được kiểm toán quyền riêng tư | Giảm sử dụng trực tiếp các bản ghi thực; xác minh rò rỉ |
| **Mô hình lớn, tập dữ liệu nhỏ** | Knowledge Distillation | Sử dụng kiến thức của mô hình "giáo viên" làm "dữ liệu" |
...
| **Kỹ thuật** | **Điều kiện tiên quyết** |
|:-----------------------------------------------------|:------------------------------------------------------------|
| **Active Learning** | Truy cập vào oracle, tập dữ liệu chưa được gán nhãn, cơ sở hạ tầng huấn luyện lại |
| **Coreset selection**\index{Coreset!hiệu quả dữ liệu} | Mô hình proxy hoặc bộ trích xuất embedding, tập dữ liệu đầy đủ có thể truy cập |
| **Curriculum Learning** | Phương pháp chấm điểm độ khó, lịch trình điều chỉnh tốc độ |
| **Bán giám sát** | Một số dữ liệu đã được gán nhãn, dữ liệu chưa được gán nhãn từ cùng phân phối |
| **Tự giám sát** | Kho ngữ liệu lớn chưa được gán nhãn, ngân sách tính toán cho pretraining |
| **Tăng cường** | Kiến thức miền về các bất biến, thư viện tăng cường |
| **Tạo tổng hợp** | Mô hình tạo sinh hoặc trình mô phỏng, giảm thiểu khoảng cách miền |
...
| **Bậc lưu trữ** |               **Thông lượng tuần tự** |             **I/O ngẫu nhiên (IOPS)** |              **Thông lượng ngẫu nhiên (xấp xỉ)** |                      **Phạt ngẫu nhiên** |
|:-----------------|:-----------------------------------------------------:|:---------------------------------------------:|:--------------------------------------------------------:|:-----------------------------------------------------------:|
...
| **Đám mây (S3)** |                Phụ thuộc vào cấu hình |              Không thể so sánh bằng IOPS |                 Phụ thuộc vào cấu hình |                     Có khả năng cực đoan |
...
| **Thành phần** |                                 **Công thức** |                        **Phạm vi minh họa** |
|:-------------------------|:----------------------------------------------------------------------------:|----------------------------------------------:|
| **$C_{\text{acquire}}$** |                         $D \times c_{\text{sample}}$                         | $0.001–$10/mẫu (cào web so với được cấp phép) |
| **$C_{\text{label}}$**   |                 $D_{\text{labeled}} \times c_{\text{label}}$                 |        $0.01–$200/mẫu (cộng đồng so với chuyên gia) |
| **$C_{\text{store}}$**   |  $D_{\text{vol,store}} \times c_{\text{storage}} \times T_{\text{months}}$   |                        $0.02–$0.10/GB/tháng |
| **$C_{\text{process}}$** | $D \times N_{\text{epochs}} \times O_{\text{sample}} \times c_{\text{FLOP}}$ |                Tỷ lệ thuận với FLOPs huấn luyện |
...
| **Thành phần chi phí** | **Tính toán** |                                        **Số lượng** |
|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------:|
| **Dữ liệu thô (`{python} CostBreakdown.n_labels_str` images)** | Tập dữ liệu được cấp phép, phí cố định |                          `{python} CostBreakdown.c_raw_usd_str`                          |
| **Nhãn (chú thích cộng đồng)** | `{python} CostBreakdown.n_labels_str` $\times$ `{python} CostBreakdown.cb_cost_per_label_str`                                                          |                         `{python} CostBreakdown.c_label_usd_str`                         |
| **Lưu trữ (kho đối tượng đám mây)** | `{python} CostBreakdown.storage_size_str` $\times$ `{python} CostBreakdown.cb_storage_rate_str` $\times$ `{python} CostBreakdown.storage_duration_str` |                         `{python} CostBreakdown.c_store_usd_str`                         |
| **Chiến dịch huấn luyện (`{python} CostBreakdown.train_run_count_str`, mỗi `{python} CostBreakdown.train_epoch_count_str` trong `{python} CostBreakdown.train_duration_str` trên `{python} CostBreakdown.train_gpu_count_str`)** | `{python} CostBreakdown.train_gpu_hours_str` $\times$ `{python} CostBreakdown.cb_gpu_rate_str`                                                         |                         `{python} CostBreakdown.c_train_usd_str`                         |
| **Tổng cộng** |                                                                                                                                                        |                       **`{python} CostBreakdown.c_total_usd_str`**                       |
| **Tỷ lệ Dữ liệu so với Tính toán** |                                                                                                                                                        | `{python} CostBreakdown.p_data_str` dữ liệu, `{python} CostBreakdown.p_compute_str` tính toán |
...
| **Kỹ thuật** | **Đầu tư (Chi phí)** | **Tiết kiệm (Lợi ích)** |
|:----------------------|:--------------------------------------------------------|:-------------------------------------------------------------|
| **Loại bỏ trùng lặp** | Tính toán một lần để băm + cơ sở hạ tầng | Giảm lưu trữ và xử lý mẫu lặp lại |
| **Lựa chọn Coreset** | Huấn luyện mô hình proxy + tính toán lựa chọn | Ít mẫu được giữ lại hơn nếu chất lượng và độ bao phủ mục tiêu được duy trì |
| **Active Learning** | Suy luận trên tập dữ liệu chưa được gán nhãn + độ trễ human-in-the-loop | Nhu cầu gán nhãn thấp hơn khi lựa chọn truy vấn được chuyển giao |
| **Tăng cường dữ liệu** | Chu kỳ CPU/GPU cho các phép biến đổi | Tăng kích thước tập dữ liệu hiệu quả mà không cần thu thập dữ liệu mới |
...
| **Thành phần** |                                                               **Chi phí** |
|:------------------------------------------|-----------------------------------------------------------------------:|
| **Xây dựng pipeline loại bỏ trùng lặp** | `{python} DeduplicationAmortization.cost_build_str` (thời gian kỹ thuật) |
| **Tính toán chữ ký MinHash (một lần)** |             `{python} DeduplicationAmortization.cost_compute_once_str` |
| **Tiết kiệm trên mỗi lần chạy** |               `{python} DeduplicationAmortization.savings_per_run_str` |
...
| **Số lần chạy** |                                                    **ROI được khấu hao** |
|:-------------------|---------------------------------------------------------------------:|
| **1 lần chạy** |            `{python} DeduplicationAmortization.roi_1_str` (lỗ ròng) |
| **5 lần chạy** |     `{python} DeduplicationAmortization.roi_5_str` (gần hòa vốn) |
| **10 lần chạy** |          +`{python} DeduplicationAmortization.roi_10_str` (dương) |
| **50 lần chạy** | +`{python} DeduplicationAmortization.roi_50_str` (lợi nhuận cao) |
...
| **Kỹ thuật** | **Giả định một Node** | **Thách thức phân tán** |
|:------------------------|:--------------------------------|:---------------------------------------------|
| **Lựa chọn Coreset** | Chế độ xem toàn cầu của tập dữ liệu | Mỗi worker chỉ thấy shard của nó |
| **Active Learning** | Chấm điểm sự không chắc chắn tập trung | Chấm điểm yêu cầu đồng bộ hóa mô hình |
| **Curriculum Learning** | Sắp xếp độ khó toàn cầu | Các worker có thể có các mẫu "khó nhất" khác nhau |
| **Loại bỏ trùng lặp** | Bảng băm (Hash table) vừa trong bộ nhớ | Các bảng băm phân tán (Distributed hash tables) làm tăng độ trễ |
...
| **Điều kiện quan sát được** | **Nút thắt cổ chai (Bottleneck) có thể xảy ra** | **Phản hồi ứng cử viên** |
|:----------------------------------------|:-------------------------------|:------------------------------------------|
| **Tốc độ đầu vào thấp hơn nhu cầu của bộ tăng tốc** | Lưu trữ, giải mã hoặc liên kết máy chủ | Sharding, tìm nạp trước, giải mã song song |
| **Các kernel huấn luyện cường độ thấp** | Băng thông bộ nhớ của bộ tăng tốc | Fusion, giảm lưu lượng, độ chính xác thấp hơn |
| **Chấm điểm động chiếm ưu thế trong lần lặp** | Tính toán lựa chọn hoặc I/O chỉ mục | Các mô hình proxy, điểm được lưu vào cache hoặc embeddings |
...
| **Hải đăng** | **Nút thắt cổ chai (Bottleneck) chính** | **Ưu tiên lựa chọn dữ liệu** |
|:---------------------|:-----------------------|:---------------------------------------------------------------------------------|
| **ResNet-50** | Tính toán | Lựa chọn coreset trực tiếp giảm FLOPs huấn luyện |
| **GPT-2/Llama** | Băng thông bộ nhớ | Loại bỏ trùng lặp giảm kích thước kho ngữ liệu; curriculum learning cải thiện hiệu quả token |
| **MobileNetV2** | Độ trễ/Công suất | Xác thực augmentation so với mô hình mục tiêu và các tính bất biến khi triển khai |
| **DLRM** | Dung lượng bộ nhớ | Loại bỏ trùng lặp tương tác và tỉa (pruning) embedding giảm kích thước bảng |
| **Phát hiện từ khóa (Keyword Spotting)** | Các ràng buộc cực đoan | Augmentation và tổng hợp tạo ra các tập dữ liệu từ các hạt giống tối thiểu |
