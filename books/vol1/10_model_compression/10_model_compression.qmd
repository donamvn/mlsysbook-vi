...
|:----------------|:--------------------:|:------------------------------------------------------------------:|:----------|:-----------------|
| **Đám mây** |      hàng chục GB |    `{python} DeploymentScenarioLatency.cloud_latency_range_str`    | Linh hoạt | Thông lượng, chi phí |
| **Di động/Edge** | hàng trăm MB đến GB | `{python} DeploymentScenarioLatency.mobile_edge_latency_range_str` | Thang W | Kích thước, độ trễ |
| **TinyML** |        KB–MB |    `{python} DeploymentScenarioLatency.tiny_latency_range_str`     | mW | Kích thước, năng lượng |
...
| **Mô hình** |               **Bộ nhớ trọng số khi runtime** |              **Lưu trữ trọng số artifact** | **Đám mây (`{python} ModelDeviceComparison.cloud_cap_gb_str`)** |   **Di động (`{python} ModelDeviceComparison.mobile_cap_gb_str`)** |      **TinyML (`{python} ModelDeviceComparison.tiny_cap_kb_str`)** |
|:-----------------------|:------------------------------------------------------:|:------------------------------------------------------:|:-------------------------------------------------------------:|:-------------------------------------------------------------------:|:------------------------------------------------------------------------:|
| **DLRM** |      `{python} ModelDeviceComparison.dlrm_gb_str`      |      `{python} ModelDeviceComparison.dlrm_gb_str`      |                               ok | không (`{python} ModelDeviceComparison.dlrm_mobile_overflow_mult_str`) |    không (`{python} ModelDeviceComparison.dlrm_tiny_overflow_mult_str`) |
| **GPT-2 XL** |      `{python} ModelDeviceComparison.gpt2_gb_str`      |      `{python} ModelDeviceComparison.gpt2_gb_str`      |                               ok |                                  ok |    không (`{python} ModelDeviceComparison.gpt2_tiny_overflow_mult_str`) |
| **ResNet-50** |     `{python} ModelDeviceComparison.resnet_mb_str`     |     `{python} ModelDeviceComparison.resnet_mb_str`     |                               ok |                                  ok |   không (`{python} ModelDeviceComparison.resnet_tiny_overflow_mult_str`) |
| **MobileNetV2** |   `{python} ModelDeviceComparison.mobilenet_mb_str`    |   `{python} ModelDeviceComparison.mobilenet_mb_str`    |                               ok |                                  ok |  không (`{python} ModelDeviceComparison.mobilenet_tiny_overflow_mult_str`) |
| **MobileNetV2 (INT8)** | `{python} ModelDeviceComparison.mobilenet_int8_mb_str` | `{python} ModelDeviceComparison.mobilenet_int8_mb_str` |                               ok |                                  ok | không (`{python} ModelDeviceComparison.mobilenet_int8_tiny_ratio_mult_str`) |
...
| **Phép cộng số thực** |                                  32-bit | `{python} QuantizationEnergyPhysics.energy_float_add_ratio_mult_str` |
| **Đọc DRAM** | `{python} QuantizationEnergyPhysics.energy_dram_read_bits_value_str`-bit | `{python} QuantizationEnergyPhysics.energy_dram_read_ratio_mult_str` |
...
|:--------------------------------------------------------|:-----------------------------------|:--------------------------------------------------------------------------------------------------|
| **Theo lớp**\index{Quantization!layerwise} | Một phạm vi cho mỗi lớp | Đơn giản nhưng không tối ưu khi các phạm vi bộ lọc thay đổi rộng rãi |
| **Theo nhóm**\index{Quantization!groupwise} | Các bộ lọc được nhóm với các phạm vi được chia sẻ | Được sử dụng trong Q-BERT [@shen2020] cho các lớp attention của transformer |
| **Theo kênh**\index{Quantization!channelwise} | Một phạm vi cho mỗi bộ lọc | Mặc định phổ biến; cân bằng lỗi lượng tử hoá với siêu dữ liệu tỷ lệ và chi phí triển khai |
| **Theo kênh con**\index{Quantization!sub-channelwise} | Các phạm vi trong mỗi bộ lọc | Các phạm vi cục bộ hơn có thể giảm lỗi lượng tử hoá nhưng làm tăng siêu dữ liệu và chi phí triển khai |
...
|:-------------------------|:----------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------|:----------------------------------------------------------------------|
| **Độ chính xác Top-1** | `{python} ResNet50Int8Metrics.fp32_top1_str`              | `{python} ResNet50Int8Metrics.int8_top1_str` (giảm `{python} ResNet50Int8Metrics.top1_drop_pp_str`) | Độ chính xác tổng hợp phần lớn được giữ nguyên, nhưng kiểm tra nhóm con vẫn quan trọng. |
| **Độ trễ V100 được mô hình hóa** | `{python} ResNet50Int8Metrics.fp32_latency_ms_str`        | `{python} ResNet50Int8Metrics.int8_latency_ms_str`                                                            | Độ trễ GPU cải thiện khi runtime ánh xạ INT8 tới các kernel nhanh. |
| **Kích thước mô hình** | `{python} ResNet50Int8Metrics.fp32_size_mb_str`           | `{python} ResNet50Int8Metrics.int8_size_mb_str` (`{python} ResNet50Int8Metrics.size_reduction_mult_str`)      | Tạo phẩm trở nên dễ dàng hơn để cache, truyền tải và triển khai trên edge. |
| **Năng lượng/suy luận** | `{python} ResNet50Int8Metrics.fp32_energy_j_str`          | `{python} ResNet50Int8Metrics.int8_energy_j_str` (`{python} ResNet50Int8Metrics.energy_improvement_mult_str`) | Độ chính xác thấp hơn làm giảm cả năng lượng tính toán số học và năng lượng di chuyển bộ nhớ. |
| **Lỗi hiệu chuẩn** | `{python} ResNet50Int8Metrics.calibration_error_fp32_str` | `{python} ResNet50Int8Metrics.calibration_error_int8_str`                                                     | Hiệu chuẩn độ tin cậy có thể bị trôi ngay cả khi độ chính xác trông có vẻ chấp nhận được. |
