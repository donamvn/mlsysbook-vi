...
|:--------------------------|:-----------------------------------------------------------:|:-----------------------------------------------------------------------:|
| **Nam giới da sáng**   |  `{python} GenderShadesDisparity.error_light_male_tbl_str`  |                          Mức cơ sở (1.0$\times$)                         |
| **Nữ giới da sáng** | `{python} GenderShadesDisparity.error_light_female_tbl_str` | `{python} GenderShadesDisparity.disparity_light_female_factor_mult_str` |
| **Nam giới da sẫm màu**    |   `{python} GenderShadesDisparity.error_dark_male_tbl_str`  |   `{python} GenderShadesDisparity.disparity_dark_male_factor_mult_str`  |
| **Nữ giới da sẫm màu**  |  `{python} GenderShadesDisparity.error_dark_female_tbl_str` |        `{python} GenderShadesDisparity.disparity_factor_mult_str`       |
...
| **Mục**                | **Nội dung**                                                                                                                                                                                                                               |
|:---------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Chi tiết mô hình**          | Kiến trúc MobileNetV2 với các tham số `{python} ModelCardExample.mv2_params_m_str`, được huấn luyện trên ImageNet sử dụng tích chập phân tách theo chiều sâu (depthwise separable convolutions). Được lượng tử hoá INT8 để triển khai trên edge.                                                    |
...
|:-----------------------|:-------------------------------------:|:-------------------------------------:|
| **Đã trả nợ (thực tế)**    | `{python} LoanFairness.a_tp_str` (TP) | `{python} LoanFairness.a_fn_str` (FN) |
| **Vỡ nợ (thực tế)** | `{python} LoanFairness.a_fp_str` (FP) | `{python} LoanFairness.a_tn_str` (TN) |
...
|                        |          **Được chấp thuận (dự đoán)**          |          **Bị từ chối (dự đoán)**          |
|:-----------------------|:-------------------------------------:|:-------------------------------------:|
| **Đã trả nợ (thực tế)**    | `{python} LoanFairness.b_tp_str` (TP) | `{python} LoanFairness.b_fn_str` (FN) |
| **Vỡ nợ (thực tế)** | `{python} LoanFairness.b_fp_str` (FP) | `{python} LoanFairness.b_tn_str` (TN) |
...
|:------------------------|:------------------------------------------:|:------------------------------------------:|:----------------------------------------------|
| **Tỷ lệ chấp thuận**       | `{python} LoanFairness.a_approval_tbl_str` | `{python} LoanFairness.b_approval_tbl_str` | `{python} LoanFairness.dp_disparity_tbl_str`  |
| **Tỷ lệ dương tính thật**  |   `{python} LoanFairness.a_tpr_tbl_str`    |   `{python} LoanFairness.b_tpr_tbl_str`    | `{python} LoanFairness.tpr_disparity_tbl_str` |
| **Tỷ lệ dương tính giả** |   `{python} LoanFairness.a_fpr_tbl_str`    |   `{python} LoanFairness.b_fpr_tbl_str`    | `{python} LoanFairness.fpr_disparity_str`     |
...
|:-----------------------|:-----------------------------------------------:|:-----------------------------------------------------------------------------------------:|:--------------------------------------------|
| **Điện thoại thông minh**         | `{python} EdgeEfficiencyCalc.smart_power_w_str` |                      `{python} EdgeEfficiencyCalc.smart_latency_str`                      | Cải thiện ảnh, trợ lý giọng nói         |
| **Cảm biến IoT**         |  `{python} EdgeEfficiencyCalc.iot_power_mw_str` |                      `{python} EdgeEfficiencyCalc.iot_latency_s_str`                      | Phát hiện bất thường, giám sát môi trường |
| **Camera nhúng**    |  `{python} EdgeEfficiencyCalc.cam_power_w_str`  | `{python} EdgeEfficiencyCalc.cam_fps_str` (`{python} EdgeEfficiencyCalc.cam_latency_str`) | Phát hiện đối tượng thời gian thực, giám sát    |
| **Thiết bị đeo**    | `{python} EdgeEfficiencyCalc.wear_power_mw_str` |                       `{python} EdgeEfficiencyCalc.wear_latency_str`                      | Giám sát sức khỏe, nhận dạng hoạt động     |
...
| **Mô hình TinyML**    | `{python} EdgeEfficiencyCalc.tiny_params_k_str` | `{python} EdgeEfficiencyCalc.tiny_power_mw_str` | `{python} EdgeEfficiencyCalc.tiny_latency_str` | `{python} EdgeEfficiencyCalc.tiny_fits_phone_str` | `{python} EdgeEfficiencyCalc.tiny_fits_iot_str` |
...
| **Thành phần chi phí**              |           **Tính toán**            |             **Chi phí tài chính**            |                  **Carbon (kg CO~2~)** |
|:--------------------------------|:------------------------------------:|:-----------------------------------------:|:--------------------------------------------------------:|
| **Chuẩn bị dữ liệu ban đầu**    |         giờ $\times$ tỷ lệ          |  `{python} TCOCalc.t_data_prep_calc_str`  |       `{python} TCOCalc.t_data_prep_carbon_kg_str`       |
| **Tìm kiếm siêu tham số**       | thử nghiệm $\times$ chi phí/thử nghiệm |    `{python} TCOCalc.t_hparam_calc_str`   |        `{python} TCOCalc.t_hparam_carbon_kg_str`         |
| **Huấn luyện cuối cùng**              |         giờ $\times$ tỷ lệ          |    `{python} TCOCalc.t_final_calc_str`    |         `{python} TCOCalc.t_final_carbon_kg_str`         |
| **Tổng phụ cho mỗi chu kỳ huấn luyện** |                                      | **`{python} TCOCalc.t_subtotal_usd_str`** |     **`{python} TCOCalc.t_subtotal_carbon_kg_str`**      |
| **Tần suất huấn luyện lại**        |      chu kỳ/năm $\times$ năm      |    `{python} TCOCalc.t_cycles_calc_str`   | hệ số nhân tương tự (`{python} TCOCalc.t_cycles_str` chu kỳ) |
| **Tổng chi phí huấn luyện**         |       tổng phụ $\times$ chu kỳ       |   **`{python} TCOCalc.t_total_usd_str`**  |       **`{python} TCOCalc.t_total_carbon_kg_str`**       |
...
| **Thành phần chi phí**        |         **Tính toán**          |           **Chi phí tài chính**           |           **Carbon (kg CO~2~)** |
|:--------------------------|:--------------------------------:|:--------------------------------------:|:-----------------------------------------:|
| **Truy vấn hàng ngày**         |   người dùng $\times$ truy vấn/người dùng    | `{python} TCOCalc.i_daily_q_calc_str`  |                     -                     |
| **GPU-giây/ngày**       | truy vấn $\times$ nhu cầu dịch vụ  | `{python} TCOCalc.i_gpu_sec_calc_str`  |                     -                     |
| **GPU-giờ/ngày**         |      giây ÷ SEC_PER_HOUR      |  `{python} TCOCalc.i_gpu_hr_day_str`   |  `{python} TCOCalc.i_daily_carbon_kg_str` |
| **Chi phí GPU hàng năm**       | giờ $\times$ 365 $\times$ tỷ lệ |  `{python} TCOCalc.i_annual_calc_str`  | `{python} TCOCalc.i_annual_carbon_kg_str` |
| **Chi phí suy luận 3 năm** |        hàng năm $\times$ 3         | **`{python} TCOCalc.i_total_usd_str`** |   **`{python} TCOCalc.i_carbon_kg_str`**  |
...
|:----------------------------------|:--------------------------------------------:|:--------------------------------------:|
| **Cơ sở hạ tầng giám sát**     | `{python} TCOCalc.o_monitor_annual_usd_str`  |  `{python} TCOCalc.o_monitor_usd_str`  |
| **Kỹ thuật viên trực (0.5 FTE)** |  `{python} TCOCalc.o_oncall_annual_usd_str`  |  `{python} TCOCalc.o_oncall_usd_str`   |
| **Phản ứng sự cố (ước tính)** | `{python} TCOCalc.o_incident_annual_usd_str` | `{python} TCOCalc.o_incident_usd_str`  |
| **Tổng chi phí vận hành**             |                                              | **`{python} TCOCalc.o_total_usd_str`** |
...
|:---------------|:------------------------------------:|:----------------------------------:|:----------------------------------------------------------:|
| **Huấn luyện**   | `{python} TCOCalc.t_total_k_usd_str` | `{python} TCOCalc.p_train_tbl_str` |          `{python} TCOCalc.t_total_carbon_t_str`           |
| **Suy luận**  |  `{python} TCOCalc.i_total_usd_str`  |  `{python} TCOCalc.p_inf_tbl_str`  |             `{python} TCOCalc.i_carbon_t_str`              |
| **Hoạt động** |  `{python} TCOCalc.o_total_usd_str`  |  `{python} TCOCalc.p_ops_tbl_str`  |                       Chưa được ước tính                        |
| **Tổng TCO**  | **`{python} TCOCalc.total_tco_str`** | **`{python} TCOCalc.p_total_str`** | **`{python} TCOCalc.total_carbon_t_str` tổng phụ được mô hình hóa** |
