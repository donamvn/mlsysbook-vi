...
|:----------------------------------------------|:-------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Nền tảng (@sec-introduction)** | Bộ ba AI | Bị giới hạn bởi các ràng buộc của máy (Pin/Nhiệt) |
| **Kiến trúc (@sec-network-architectures)** | Hiệu quả thuật toán | Tích chập tách sâu: `{python} MobileNetJourneyMath.depthwise_reduction_mult_str` ít FLOPs hơn cho một lớp 3x3, 256 kênh đầu ra đại diện và `{python} MobileNetJourneyMath.resnet_ops_reduction_mult_str` ít phép toán hơn so với ResNet-50 ở quy mô ImageNet |
| **Huấn luyện (@sec-model-training)** | Thông lượng so với Độ trễ | Tối ưu hóa cho độ trễ di động một yêu cầu; tăng cường dữ liệu có thể cải thiện độ bền vững |
| **Nén (@sec-model-compression)** | Điều hướng Biên Pareto | Lượng tử hoá INT8: FP32 sử dụng `{python} MobileNetJourneyMath.int8_fp32_reduction_mult_str` và FP16 sử dụng `{python} MobileNetJourneyMath.int8_fp16_reduction_mult_str` dung lượng lưu trữ trên mỗi giá trị của INT8, với độ chính xác được xác thực lại theo từng triển khai |
| **Tăng tốc (@sec-hardware-acceleration)** | Tôn trọng Hợp đồng Silicon | Ánh xạ các kernel đến các NPU di động (ví dụ, Apple Neural Engine) để tối đa hóa việc sử dụng phần cứng |
| **Phục vụ (serving) (@sec-model-serving)** | Tôn trọng Ngân sách Độ trễ | Ràng buộc $\text{p99} < 50$ ms; tối ưu hóa tiền xử lý (thay đổi kích thước/chuẩn hóa) để tránh tắc nghẽn CPU |
| **Vận hành (@sec-ml-operations)** | Quản lý Entropy Hệ thống | Giám sát trôi dạt: Phát hiện sự dịch chuyển phân phối và theo dõi độ chính xác cấp độ nhóm trên các quần thể thiết bị không đồng nhất và điều kiện ánh sáng |
...
|:--------------------------------|:---------------------------------|:---------------|:-----------------------------------------------------------------------------------------------------------------------------|:--------------------------------------------------------|
| \ref{pri-data-as-code} | Nguyên tắc Dữ liệu là Mã | I: Nền tảng | Behavior $=f$(data, algorithm, code, randomness) | Dữ liệu thay đổi hành vi; các đầu vào khác cũng quan trọng |
| \ref{pri-data-gravity} | Nguyên tắc Trọng lực Dữ liệu | I: Nền tảng | Di chuyển tính toán về phía dữ liệu khi chi phí truyền tải lặp lại vượt quá chi phí đặt chỗ | Phụ thuộc vào khối lượng, tái sử dụng, mạng và khả năng di chuyển của tính toán |
| \ref{pri-iron-law} | Luật Sắt của Hệ thống ML | II: Xây dựng | $T_{\text{seq}}=D_{\text{vol}}/\text{BW}+O/(R_{\text{peak}}\eta_{\text{hw}})+L_{\text{lat}}$; sự chồng chéo dao động từ tối đa đến tổng | Các giai đoạn có thể cộng thêm hoặc chồng chéo |
| \ref{pri-silicon-contract} | Hợp đồng Silicon | II: Xây dựng | $I_{\text{ridge}}=R_{\text{peak}}/\text{BW}$; so sánh $I_{\text{model}}$ với $I_{\text{ridge}}$ | Chẩn đoán hoạt động bị giới hạn bởi băng thông so với tính toán |
| \ref{pri-pareto-frontier} | Biên Pareto | III: Tối ưu hóa | $\nexists c'\ne c:\,[\forall k\,M_k(c')\ge M_k(c)]\land[\exists j\,M_j(c')>M_j(c)]$                                          | Không có cấu hình riêng biệt nào chiếm ưu thế tại một điểm biên |
| \ref{pri-arithmetic-intensity} | Định luật Cường độ Số học | III: Tối ưu hóa | $R_{\text{attain}} \le \min(R_{\text{peak}},\; I \times \text{BW})$                                                          | Nhiều tính toán hơn không thể nâng cao giới hạn băng thông |
| \ref{pri-energy-movement} | Bất biến Năng lượng-Chuyển động | III: Tối ưu hóa | $E_{\text{total}}=\sum_j N_jE_j$; tỷ lệ chi phí DRAM/FLOP: `{python} EnergyMovementMath.dram_vs_range_mult_str` | Tổng năng lượng phụ thuộc vào số lượng và chi phí sự kiện |
| \ref{pri-amdahl} | Định luật Amdahl | III: Tối ưu hóa | $\text{Speedup} = \frac{1}{(1-f_{\text{parallel}}) + \frac{f_{\text{parallel}}}{S_{\text{parallel}}}}$                       | Phần tuần tự giới hạn tất cả các lợi ích song song |
| \ref{pri-verification-gap} | Khoảng cách Xác minh | IV: Triển khai | $\Pr_{(X,Y)\sim P_{\text{deploy}}}[d(f(X),Y)\le\tau]\ge1-\epsilon$                                                           | Chỉ định khoảng cách, dung sai, quần thể và độ tin cậy |
| \ref{pri-statistical-drift} | Chẩn đoán Trôi dạt Thống kê | IV: Triển khai | $\text{Accuracy}(t)\approx\text{Accuracy}_0-\lambda\mathcal{D}(P_t\Vert P_0)$                                                | Một sự phù hợp cục bộ; sự trôi dạt không nhất thiết làm giảm chất lượng |
| \ref{pri-training-serving-skew} | Chẩn đoán Lệch Huấn luyện-Phục vụ | IV: Triển khai | $S_{\text{skew}}=\mathbb{E}_{X\sim P_{\text{deploy}}}[d(f_{\text{serve}}(X),f_{\text{train}}(X))]$                           | Sự không khớp đầu ra báo hiệu rủi ro, không phải mất độ chính xác |
| \ref{pri-latency-budget} | Nguyên tắc Ngân sách Độ trễ | IV: Triển khai | $T_q\le L_{\text{budget}}$                                                                                                   | SLO của sản phẩm chọn $q$ và ngân sách của nó |
| \ref{pri-bias-feedback} | Mô hình Phản hồi Độ chệch (Bias) | IV: Triển khai | $\Delta_g(k)\approx\Delta_g(0)\alpha_{\text{fb}}^k$ với $\alpha_{\text{fb}}$ đã được khớp | Phản hồi có thể khuếch đại tác hại; đo lường và can thiệp |
