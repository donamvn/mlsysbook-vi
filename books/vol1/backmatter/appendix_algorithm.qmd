...
| **Loại lớp** |        **Hình dạng đầu ra** |                                       **Tham số ($P$)** |                              **FLOPS (mỗi lượt chuyển tiếp)** |
|:------------------------------|:-----------------------------:|:------------------------------------------------------------------------------------------------:|:--------------------------------------------------------------------------------------:|
| **Tuyến tính** |     $(B, N_{\text{out}})$     |            $N_{\text{in}} \times N_{\text{out}} + N_{\text{out}}$ nếu độ chệch (bias) được bật |                $2 \times B \times N_{\text{in}} \times N_{\text{out}}$                 |
| **Tích chập 2D** | $(B, C_{\text{out}}, H', W')$ |       $K^2 \times C_{\text{in}} \times C_{\text{out}} + C_{\text{out}}$ nếu độ chệch (bias) được bật | $2 \times B \times H' \times W' \times K^2 \times C_{\text{in}} \times C_{\text{out}}$ |
| **Multi-Head Self-Attention** |   $(B, S, d_{\text{model}})$  | $4 \times d_{\text{model}}^2$, cộng thêm $4 \times d_{\text{model}}$ nếu độ chệch (bias) chiếu được bật |              $B \times (4 S^2 d_{\text{model}} + 8 S d_{\text{model}}^2)$              |
| **LayerNorm** |   $(B, S, d_{\text{model}})$  |                 $2 \times d_{\text{model}}$ nếu tỷ lệ affine và độ chệch (bias) được bật |                   $\mathcal{O}(B \times S \times d_{\text{model}})$                    |
